...
| `supervisor@toko.com` | web | 11 | 15 | FAIL |
| `teknisi@toko.com` | mobile-only gate | — | 1 | PASS |
| `cs@toko.com` | mobile-only gate | — | 1 | PASS |
| `admin_staff@toko.com` | web | 3 | 4 | FAIL |
| `kasir@toko.com` | web | 4 | 7 | FAIL |
| `gudang@toko.com` | mobile-only gate | — | 1 | PASS |

Ringkasan Playwright: **7 akun PASS, 3 akun FAIL**. Tidak ditemukan respons server 5xx. Seluruh kegagalan akun berasal dari satu defect bersama pada `/app/my-kpi/daily`: `Input is not defined`.
...
- Status: RED terkonfirmasi ulang setelah `npm run build`; belum diperbaiki dalam audit ini.
...
- Dampak: pembaruan notifikasi/dashboard realtime tidak bekerja pada konfigurasi lokal yang diuji.
...
| 5 | KPI Saya dapat dirender tanpa exception browser | test yang sama | E2E | FAIL |
| 6 | Backend workflow dan otorisasi existing tetap hijau | `php artisan test --compact` | unit/integration | PASS — 183 test, 1.165 assertion |
...
accounts: 10; passed: 7; failed: 3
FAIL: /app/my-kpi/daily: Input is not defined

php artisan test --compact
Tests: 183 passed (1165 assertions)

flutter test
All tests passed! (19 tests)

npm run build
3001 modules transformed; built in 3.59s
...
- Belum ada GREEN evidence untuk dua temuan RED karena permintaan ini adalah audit/testing, bukan implementasi perbaikan.

## Merge evidence

- RED: Playwright gagal konsisten pada KPI Saya untuk tiga akun dan mencatat kegagalan koneksi realtime pada semua akun.
- GREEN parsial: 7 akun, seluruh matriks menu, route denial, 183 test backend, 19 test Flutter, dan build frontend lulus.
- Refactor: tidak ada.
