...
export default function KpiCorrectionForm({ kpi }) {
    const form = useForm({ reason: '', items: kpi.items.map((item) => ({ id: item.id, actual: item.actual ?? '' })), evidence: [{ type: 'document', reference: '' }] });
        ...
        <>
            ...
            <div className="min-h-[calc(100dvh-76px)] bg-muted/15 px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto max-w-3xl space-y-6">
                ...
                <Card><CardHeader className="border-b border-border/70"><CardTitle>Koreksi resmi</CardTitle><CardDescription>Koreksi akan masuk proses dual authorization dan tidak langsung mengubah KPI.</CardDescription></CardHeader><CardContent><form onSubmit={submit} className="space-y-6">
                    ...
                    <div><label htmlFor="evidence-reference" className="mb-2 block text-sm font-medium">Evidence koreksi</label><Input id="evidence-reference" value={form.data.evidence[0]?.reference ?? ''} onChange={(event) => form.setData('evidence', [{ ...form.data.evidence[0], reference: event.target.value }])} placeholder="Referensi dokumen / tautan bukti" />{form.errors.evidence && <p className="mt-1.5 text-xs font-medium text-destructive">{form.errors.evidence}</p>}</div>
