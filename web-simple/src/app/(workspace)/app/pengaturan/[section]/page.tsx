...
export default async function AdminSettingsPage({ params, searchParams }: {
  ...
  searchParams: Promise<{ accountId?: string; positionId?: string }>;
}) {
  ...
  if (section === "pengguna") return <UsersSection accountId={query.accountId} />;
    ...
    {branches.length ? <section className="panel">
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Cabang</th><th>Pegawai</th><th>Status</th><th></th></tr></thead><tbody>
        {branches.map((branch) => <tr key={branch.id}>
          <td><span className="cell-title">{branch.name}</span><span className="cell-subtitle">{branch.code}</span></td>
          <td>{branch._count.employees}</td>
          <td><StatusBadge status={branch.isActive ? "ACTIVE" : "INACTIVE"} /></td>
          <td><BranchDialog branch={branch} trigger={<Button variant="secondary" size="small"><PencilSimpleIcon aria-hidden="true" />Edit</Button>} /></td>
        </tr>)}
      </tbody></table></div>
...
async function PositionsSection() {
  ...
  return <><PageHeader eyebrow="Super Admin" title="Jabatan" description="Jabatan yang dinilai KPI otomatis memperoleh template draft untuk dilengkapi." actions={<PositionDialog trigger={<Button><PlusIcon aria-hidden="true" />Tambah jabatan</Button>} />} />
    {positions.length ? <section className="panel"><div className="panel-header"><div><h2>Daftar jabatan</h2><p>{positions.length} jabatan tersimpan</p></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Jabatan</th><th>Pegawai</th><th>Template KPI</th><th>Status</th><th></th></tr></thead><tbody>{positions.map((position) => {
      ...
      return <tr key={position.id}><td><span className="cell-title">{position.name}</span><span className="cell-subtitle">{position.code}</span></td><td>{position._count.employees}</td><td>{position.isKpiSubject ? <><StatusBadge status={active ? "ACTIVE" : draft ? "DRAFT" : null} /><span className="cell-subtitle">{version ? `v${version.versionNumber} · ${formatNumber(total)}%` : "Belum tersedia"}</span></> : "Tidak dinilai"}</td><td><StatusBadge status={position.isActive ? "ACTIVE" : "INACTIVE"} /></td><td><div className="table-actions">{position.isKpiSubject && position.template ? <Button asChild variant="secondary" size="small"><Link href={`/app/pengaturan/indikator?positionId=${position.id}`}>Atur indikator</Link></Button> : null}<PositionDialog position={position} trigger={<Button variant="secondary" size="small"><PencilSimpleIcon aria-hidden="true" />Edit</Button>} /></div></td></tr>;
...
async function UsersSection({ accountId }: { accountId?: string }) {
  const [branches, positions, users, employees] = await Promise.all([
...
async function IndicatorsSection({ positionId }: { positionId?: string }) {
  const positions = await prisma.position.findMany({
    ...
    orderBy: { name: "asc" },
    include: { template: { include: { versions: { where: { status: { in: ["ACTIVE", "DRAFT"] } }, orderBy: { versionNumber: "desc" }, include: { indicators: { orderBy: [{ sortOrder: "asc" }, { code: "asc" }] } } } } } },
  });
...
function IndicatorDialog({ versionId, indicator, trigger }: { versionId: string; indicator?: KpiIndicator; trigger: React.ReactNode }) {
  const identity = <>
    <input type="hidden" name="versionId" value={versionId} />
    {indicator ? <input type="hidden" name="id" value={indicator.id} /> : null}
    <div className="form-grid"><Field label="Kode indikator" name="code" defaultValue={indicator?.code} required /><Field label="Nama indikator" name="name" defaultValue={indicator?.name} required /></div>
    ...
    <div className="form-grid"><SelectField label="Jenis nilai" name="kind" defaultValue={indicator?.kind ?? "NUMERIC"} options={indicatorKindOptions} required /><Field label="Satuan" name="unit" defaultValue={indicator?.unit} required /></div>
  </>;
  const calculation = <div className="form-grid">
    ...
    <SelectField label="Arah skor" name="direction" defaultValue={indicator?.direction ?? "HIGHER"} options={directionOptions} required />
    <Field label="Target" name="target" type="number" step="any" defaultValue={indicator?.target.toString() ?? "0"} required />
    <Field label="Batas gagal (untuk lower)" name="failureLimit" type="number" step="any" defaultValue={indicator?.failureLimit?.toString() ?? ""} />
    <Field label="Bobot (%)" name="weight" type="number" step="0.01" min={0.01} max={100} defaultValue={indicator?.weight.toString()} required />
    <Field label="Urutan" name="sortOrder" type="number" min={1} max={999} defaultValue={indicator?.sortOrder ?? 1} required />
  </div>;
  return <FormDialog trigger={trigger} title={indicator ? `Edit ${indicator.name}` : "Tambah indikator"} description="Isi identitas indikator, lalu tentukan cara perhitungannya." size="large">
    <WizardActionForm action={saveIndicatorAction} steps={[{ title: "Identitas", content: identity }, { title: "Perhitungan", content: calculation }]} submitLabel={indicator ? "Simpan perubahan" : "Tambah indikator"} />
  </FormDialog>;
}
...
const aggregationOptions: SelectOption[] = [{ value: "SUM", label: "Jumlah" }, { value: "AVERAGE", label: "Rata-rata" }];
const directionOptions: SelectOption[] = [{ value: "HIGHER", label: "Lebih tinggi lebih baik" }, { value: "LOWER", label: "Lebih rendah lebih baik" }, { value: "ZERO_TOLERANCE", label: "Harus nol" }];
...
function RatingTable({ bands }: { bands: Array<{ id: string; code: string; label: string; minScore: { toString(): string } }> }) {
  return <div className="table-wrap"><table className="data-table"><thead><tr><th>Kode</th><th>Predikat</th><th>Nilai minimum</th></tr></thead><tbody>{bands.map((band) => <tr key={band.id}><td>{band.code}</td><td><strong>{band.label}</strong></td><td>{band.minScore.toString()}</td></tr>)}</tbody></table></div>;
}

async function PeriodsSection() {
  const [periods, evaluatedPositions, activeScheme] = await Promise.all([
    prisma.kpiPeriod.findMany({ orderBy: [{ year: "desc" }, { month: "desc" }] }),
    prisma.position.findMany({ where: { isActive: true, isKpiSubject: true, employees: { some: { status: "ACTIVE" } } }, orderBy: { name: "asc" }, include: { template: { include: { versions: { where: { status: "ACTIVE" }, select: { id: true } } } } } }),
