...
import { SearchField, SelectField } from "@/components/ui/form-controls";
...
export default async function RecapPage({ searchParams }: { searchParams: Promise<{ periodId?: string; status?: string; page?: string; q?: string }> }) {
  ...
  const resultWhere = period ? {
    periodId: period.id,
    ...monthlyKpiScope(user),
    ...(status ? { status } : {}),
    ...(q ? { OR: [
      ...
      { positionCodeSnapshot: { contains: q, mode: "insensitive" as const } },
