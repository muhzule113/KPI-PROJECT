...
try {
  const users = await prisma.user.findMany({
    where: { username: { in: AUGUST_2026_ROSTER.map((employee) => employee.username) } },
  ...
  for (const expected of AUGUST_2026_ROSTER) {
    const user = users.find((candidate) => candidate.username === expected.username);
  ...
  const results = await prisma.monthlyKpi.findMany({
    where: { periodId: period.id, employee: { user: { username: { in: AUGUST_2026_ROSTER.map((employee) => employee.username) } } } },
  ...
  console.table([...results]
    .sort((left, right) => AUGUST_2026_ROSTER.findIndex((employee) => employee.username === left.employee.user.username) - AUGUST_2026_ROSTER.findIndex((employee) => employee.username === right.employee.user.username))
    .map((result) => ({
