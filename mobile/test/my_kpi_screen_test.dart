...
void main() {
  setUp(() {
    ...
    ApiService.client = MockClient((request) async {
      ...
      if (path.endsWith('/my-kpi/active')) {
        return http.Response(jsonEncode({'success': true, 'data': _kpiData()}), 200,
            headers: {'content-type': 'application/json'});
      }
      if (path.contains('/my-kpi/items/')) {
        return http.Response(jsonEncode({'success': true, 'data': _itemDetail()}), 200,
            headers: {'content-type': 'application/json'});
      }
  ...
  testWidgets('My KPI menampilkan indikator tanpa tombol submit & tanpa input manual', (tester) async {
  ...
  testWidgets('Detail indikator read-only — tidak ada field input nilai aktual', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: MyKpiScreen()));
    await tester.pumpAndSettle();
    ...
    expect(find.textContaining('Dihitung otomatis oleh sistem'), findsOneWidget);
  });
}
